...
const SidebarItem = ({ chat, index }: Props) => {
    ...
    const { updateChat, deleteChat } = useMockData();
    ...
    const handleUpdateTitle = async () => {
        try {
            if (!chat?.id || !title.trim()) return;
            
            updateChat(chat.id, { title: title.trim() });
            setIsEditable(false);
    ...
    return (
        <Container delay={0.5 * index + 0.1}>
            <li className="relative">
                <Link href={`/chat/${chat?.id}?mode=ova`} className="pl-2.5 pr-1 flex items-center justify-between py-2 rounded-lg cursor-pointer hover:bg-neutral-200/40 w-full group/id">
