...
import { ArrowUpIcon, CheckIcon, Loader2Icon, PlusIcon, SearchIcon, XIcon } from "lucide-react";
...
const ChatInput = ({ isLoading, handleSendMessage, onSearch }: Props) => {
    ...
    const transcribeAudio = async (audioBlob: Blob) => {
        try {
            ...
            // Frontend-only: Simulate transcription delay
            await new Promise(resolve => setTimeout(resolve, 1500));
            
            // Mock transcription - in a real app, this would call the API
            toast.info("Voice transcription is disabled in demo mode");
            setInput(""); // Clear input since we can't actually transcribe
    ...
    const placeholder = isDriveMode 
        ? "Search your conversations..." 
        : "Type a message...";

    const buttonIcon = isDriveMode ? (
    ...
    const buttonTooltip = isDriveMode 
        ? "Search conversations (Enter)" 
        : "Send message (Enter)";

    const isDisabled = isLoading || isRecording || isTranscribing || (isDriveMode && isSearching);
                ...
                <div className="flex flex-1 gap-4 mx-auto text-base md:gap-5 lg:gap-6 md:max-w-3xl">
                    ...
                    >
                        ...
                        >
                            ...
                            {/* mic - only in OVA mode */}
                            {!isDriveMode && micSupported && (
                                <div
                                    className={cn(
